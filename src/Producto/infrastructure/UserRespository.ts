...
export class productoRepositori implements ProductoRepository {
  ...
  async putUser(Nombre: string, Telefono: string, Correo: string): Promise<Producto> {
    ...
    return await new Promise((resolve, reject) => {
      const sql = `UPDATE Restaurantdb.Users SET Nombre='${Nombre}', Telefono='${Telefono}' WHERE Correo='${Correo}'`;
  ...
  async deleteById(Correo: string): Promise<Producto | null> {
    const mysql = new database();
    return await new Promise((resolve, reject) => {
      mysql.connection.query(`DELETE FROM Restaurantdb.Users WHERE Correo = '${Correo}'`, (error: QueryError, rows: Producto) => {
        if (error) {
          reject(error);
        } else {
          resolve(rows)
  ...
  async getById(Correo: string): Promise<Producto | null> {
    const mysql = new database();
    return await new Promise((resolve, reject) => {
      mysql.connection.query(`SELECT * FROM Restaurantdb.Users WHERE Correo = '${Correo}';`, (error: QueryError, rows: Producto) => {
        if (error) {
          reject(error);
        } else {
          resolve(rows)
  ...
  async getUser(): Promise<Producto[] | null> {
    const mysql = new database();
    return await new Promise((resolve, reject) => {
      mysql.connection.query("SELECT * FROM Restaurantdb.Users", (error: QueryError, rows: Producto[]) => {
        if (error) {
          reject(error);
        } else {
          resolve(rows)
  ...
  async create(Nombre: string, Telefono: string, Correo: string): Promise<Producto> {
    const mysql = new database();
    return await new Promise((resolve, reject) => {
      const sql = `INSERT INTO Restaurantdb.Users(Nombre,Telefono,Correo) VALUES ('${Nombre}','${Telefono}','${Correo}')`;
      mysql.connection.query(sql, (error: QueryError, results: Producto) => {
        if (error) {
          reject(error)
        } else {
